<vacancy-response class="section vacancy__response container">

    <div class="row">
        <div class="col-md-offset-12 col-md-12">
            <div class="vacancy__response__block">
                <div class="vacancy__response__button btn btn-primary-hover-success btn-shadow btn-l">
                ...
                <div class="btn-icon" data-balloon="Добавить в избранное">
                    <icon-like color="blueBright" size="m"></icon-like>
                </div>
                <a target="_blank" if={ opts.item.address.city && opts.item.address.street } href="https://yandex.ru/maps/?text={ opts.item.address.city }, { opts.item.address.street }, { opts.item.address.building }" class="btn-icon" data-balloon="Адрес на карте">
                    <icon-location color="green" size="m"></icon-location>
                </a>
                <div class="vacancy__response__salary">
                    <div class="vacancy__response__salary__title">Уровень зарплаты</div>
                    <div if={ opts.item.salary } class="vacancy__response__salary__text nowrap">{ 'от' : opts.item.salary.from } { parent.opts.utils.numberFormat(opts.item.salary.from, 0, ".", " ") } <span if={ opts.item.salary.to }>до { parent.opts.utils.numberFormat(opts.item.salary.to, 0, ".", " ") }</span> { currency(opts.item.salary.currency) }</div>
                    <div if={ !opts.item.salary } class="vacancy__response__salary__text">з/п не указана</div>
                </div>
            </div>
        </div>
    </div>
